...
class Infos:
	def __init__(self, subject: Literal['player', 'opponent'], left: float, top: float, monster_manager: MonsterManager) -> None:
		...
		self.ratio = self.health_rect.width / self.__get_monster().max_health
	...
	def __draw_health_bar ( self ):
		progress_bar = pygame.FRect(self.health_rect.left, self.health_rect.top, self.__get_monster().health * self.ratio, self.health_rect.height)
